...
| the icon | `Resources/AppIcon.icon` (re-export from Icon Composer, never hand-edit `icon.json`), `Resources/previews/ShiftPick-preview-1024.png` (rendered by `ictool`), `docs/assets/icon.png`, `Resources/ICON-NOTES.md`; the menu-bar mark is the same drawing in code, `App/MenuBarController.icon()`, and the wizard's accent is the icon's blue, `OnboardingWindow.brand` | `architecture.md` *Build and signing*, `functional.md` §6 |
