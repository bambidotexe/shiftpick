...
// own localization helper (ShiftPick: an accessor on Core/StringsOnboarding.swift that switches over
// Language, so English and French are added together or not at all).
// (In ShiftPick the live copy is Sources/ShiftPickApp/OnboardingWindow.swift.)
...
enum OnboardingPage {
    ...
    ///
    /// `height` is fitted to how many rows the page carries, because one number cannot serve two and five:
    /// `Metrics.listHeight` (560) was fitted to a page of five rows, `Metrics.shortListHeight` (440) to a
    /// page of one or two. Fitted by eye in the running window, like every other number here.
    case list(header: String, intro: String, items: [GrantItem], advanceWhen: ([GrantItem]) -> Bool,
              height: CGFloat = Metrics.listHeight)
