...
// localization helper (ShiftPick: an accessor on Core/StringsOnboarding.swift that switches over Language,
// so English and French are added together or not at all).
// (In ShiftPick the live copies are Sources/ShiftPickApp/GrantCatalogue.swift and OnboardingWindow.swift.)
