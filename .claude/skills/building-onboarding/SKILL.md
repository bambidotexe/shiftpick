...
| You are in | Do this |
|---|---|
| **ShiftPick** | The window exists. Change it there: `Sources/ShiftPickApp/OnboardingWindow.swift` (the controller, the pages and `GrantRow`), `GrantCatalogue.swift` (what a grant is, `FocusReturnWatch`, the rows), `ControlActionHandler.swift`. **The words are not in those files**: they are `Sources/ShiftPickCore/StringsOnboarding.swift`. This file is the contract the window keeps. |
...
Accessibility, Screen Recording, Input Monitoring, Notifications and the login item alike. Leave it nil only
for a flow that stays inside the app (writing a config file, a sudoers rule, `SMAppService.mainApp.register()`).
...
|---|---|---|
| `AXIsProcessTrustedWithOptions` | Device Control and Data Access | Contrôle de l’appareil et accès aux données |
...
3. **A name the whole world still uses can be gone from the pane.** `SecurityPrivacyExtension.appex`'s
   `ACCESSIBILITY` key reads **Device Control and Data Access** on macOS 27, and no key in that table
   answers "Accessibility" any more, although the API, the system's own dialog and every article about it
   still say Accessibility. The pane wins for the row's **title**; the `why` beside it is where the word the
   user came looking for goes.

Every name goes in both languages, through the project's localization helper, with the OS's own punctuation
(macOS writes a curly apostrophe: `accès aux données`, `Surveillance de l'entrée`).
...
1. **Requesting at startup.** A line in the app's start-up path that calls a request API, often guarded by
   something like "only once onboarding is done", or by "the permission is missing anyway". If the user
   reached the end of onboarding without granting that row, the grant is still `notDetermined`, and the next
   launch prompts them out of nowhere.
...
| Job | Use | Never |
|---|---|---|
| read, on every poll tick | `AXIsProcessTrusted()`, `CGPreflightScreenCaptureAccess()`, `IOHIDCheckAccess(...)`, `getNotificationSettings`, `SMAppService...status` | any `request`/`Request` call |
| ask, from a button only | `AXIsProcessTrustedWithOptions([kAXTrustedCheckOptionPrompt: true])`, `CGRequestScreenCaptureAccess()`, `IOHIDRequestAccess(...)`, `requestAuthorization(...)`, `register()` | anywhere but a control's action |
...
```swift
action: { _, done in Permissions.requestAccessibility(); done() }         // right
action: { _, done in if !request() { openSystemSettings() }; done() }     // the double-open bug
```

The one exception is a grant macOS offers **no dialog for**, where the pane *is* the flow. A bundled agent's
Background App Activity is the only one today, and the button says what it does: "Open Login Items Settings".
...
- **That poll is the app's only one, and the window owns it.** A second timer somewhere else, watching the
  same grant for the app's own sake, is one timer too many: give the window's tick a callback instead. A
  timer started when the window opens and stopped only when the grant arrives outlives the window the
  documentation says it belongs to.
...
- **Heights**: hero 440, final 400, and a list page's is fitted to how many rows it carries: **560 for five
  rows, 440 for one or two**. The window resizes around its **top-left** corner, so the title bar stays put
  and the page grows downward.
...
- It opens on a **first run**, whatever the grants are, and again whenever a grant the app cannot work
  without has gone. One stored flag says the last page's button was pressed; the wizard **does not close
  itself** when a grant arrives, because that is the one run on which the user never sees the pitch or the
  last page. The row ticks over, the button turns from Skip to Continue, and they walk on.
...
- A grant catalogue is **one list, shared** with the Settings window where both report the same state
  (KoffeeLid: `PermissionCatalog`, `HookCatalog`). The onboarding's words are in the item; a Settings page
  words the same state its own way. One list means a flow fixed once is fixed in both places. **ShiftPick's
  two lists are its own**, because its Settings System page reports the one permission with the kit's
  `StatusRow` and says nothing the catalogue could answer for; a second grant on either side is the moment
  to merge them.
...
swift build                                                          # the truth; editor diagnostics are stale
swift test                                                           # TWO summary lines. Count them.
rg -n 'level *=|collectionBehavior' Sources/ShiftPickApp/OnboardingWindow.swift          # nothing
rg -n 'activate\(ignoringOtherApps' Sources/ShiftPickApp/                                # only where the table above allows it
rg -n 'requestAccessibility' Sources/                                                    # only a control's action
rg -n '[—–‒―‐‑−]' Sources/ShiftPickCore/StringsOnboarding.swift                          # no long dashes in user text
```
...
| Keep a second timer of the app's own on the same grant | One poll, owned by the window, with a callback for the app. |
| Close the wizard the moment the grant arrives | The row ticks over and the button turns to Continue. Closing it is the user's move. |
