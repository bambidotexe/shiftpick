...
description: Use when adding, removing, renaming, moving or rewording anything in ShiftPick's Settings window (Sources/ShiftPickApp/Settings*.swift) - a setting, a switch, a status, a group, a page, a hint, a warning, a note, a button, any sentence the window shows - when a new feature needs a user-facing setting, or when building a settings or preferences window for a new macOS app that must look and read like SnappySnap's.
---
...
| You are in | Do this |
|---|---|
| **ShiftPick** | The kit exists. Build from `Sources/ShiftPickApp/SettingsKit.swift`. Never write a `Form`, a `Section`, a `GroupBox` or a hand-made row. Go to *Evolving the window*. |
...
## Evolving the window (ShiftPick)

Files, all in `Sources/ShiftPickApp/`: `SettingsKit.swift` (the kit, `SettingsMetrics`),
`SettingsWindow.swift` (the toolbar window, and the height that follows the page),
`SettingsView.swift` (`SettingsPageID`: the pages, their titles and symbols; `SystemStatus`, the two
facts the window reports but does not own), `Settings…Page.swift` (one per page, structure only).

**The words are not in the page files.** A page's copy is `Sources/ShiftPickCore/Strings<Page>Page.swift`,
and the shared status vocabulary and the three page titles are `Core/StringsSettings.swift`. Every string
is one accessor switching over `Language`, so English and French are added together or not at all, and
`LocalizationTests` reads them back: no long dash anywhere, and a key is its symbol then its name.

ShiftPick has three pages: **General**, **Selection** (its one feature), **System**. There is no fourth
subject, and a fourth page would mean the app had grown a second thing to do.

**Adding a setting:**
1. Pick the page by subject and the group by what it governs. A new group only if no group's explanation
   covers it; a new page only if the subject has two groups and fits nowhere.
2. Where it is stored. A value of the app's own: a property on `Core/Settings.swift`, which is one JSON
   blob in `UserDefaults` and decodes every key tolerantly, so an older file never resets the rest.
   What the system owns: read it from the system every time (`LoginItem`, `Permissions`), never mirrored.
3. The row, from the kit, bound through `SettingsStore`. If it depends on a switch, put it under it in the
   same card with `enabled:`.
4. The words, by the ten rules. Ask of each hint and note: would the owner call it useless?
5. `docs/functional.md` §5, and a line in `MANUAL_TESTS.md` §12.

**Changing how anything looks or reads** (a number, a colour, a control, the structure, a page split): only
the owner can judge it, in the running app. Do not judge the look yourself and do not commit before the
owner says it is done.

## Verify

```sh
swift build
rg -n '[—–‒―‐‑−]' Sources/ShiftPickCore/Strings*.swift   # nothing; LocalizationTests fails on one
rg -n '\.font\(\.(caption|callout|footnote|subheadline|title)' Sources/ShiftPickApp/Settings*.swift   # nothing
rg -n 'radioGroup|Form \{|Section|GroupBox|TabView' Sources/ShiftPickApp/Settings*Page.swift            # nothing
swift test --filter LocalizationTests                                                                    # the dash and key-symbol rules
```

Then `make install`, open Settings from the menu bar item (⌘,) and let the owner look. The window has no
automated test; `swift run axdump tree $(pgrep -x ShiftPick) 6` prints what is actually on screen, which is
how a truncated sentence was caught once already.
