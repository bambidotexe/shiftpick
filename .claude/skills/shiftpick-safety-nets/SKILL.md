...
| 4 | **The breaker.** `K.breakerTrips` (3) timeouts inside `K.breakerWindow` (60 s) destroy both taps, and only `tryAgain` (the System page's *Start Listening Again* button) creates them again. A disable **for user input** is the app's own disable heard back and is never counted (`docs/pitfalls.md` 15) | `TapLifecycle.tripped`, `disabledForUserInput` | `TapLifecycleTests`; `SafetyNetTests.testTheSafetyNumbersKeepTheirOrder` |
...
| 11 | **Asleep, locked, or another user's session**: suspended (disarmed, nothing arms) until every reason it was away has gone. The reasons are held apart, each ended by its own notification, and checked against the session itself (`CGSessionCopyCurrentDictionary`) at any news; **the last to end resumes, whichever it is and in whichever order**, and a ⇧ Shift press while away is answered from the session alone. A set looked at only once it was empty left the engine suspended for good (`docs/pitfalls.md` 17) | `Core/AwayReasons.swift`; `AppDelegate.watchTheSession` feeds it and does what it says | `AwayReasonsTests`; `TapLifecycleTests`; `TapLifecycleInvariantTests` 10; `SafetyNetTests.testTheSessionIsReconciledThroughOneValue` |
...
| `away (asleep); nothing arms until it is over` … `back (locked); nothing is away any more` | the Mac went to sleep, locked or changed session, and came back: suspended at the first reason, resumed at the last. **A `still away: locked` line with no line ending `nothing is away any more` after the screen is unlocked is `docs/pitfalls.md` 17**, and so is a ⇧ Shift click that does nothing with every window saying it is listening |
