...
| | `CrashReports`, `ProcessStats`, `FinderProcess` | The Health page's readers: this app's crash reports, its uptime and memory, and whether Finder is running. Each answers at once, asks nothing that can block, and asks for nothing. |
...
- **Nothing polls while idle.** With the permission granted and no window open, the only timer armed is the
  ...
  already on the main actor, the permission from the window's poll, and its own readings
