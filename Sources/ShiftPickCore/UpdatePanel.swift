...
/// button. The button looks for a release, and once a newer one is known it opens the update window
/// instead. What a check nobody asked for finds shows here exactly as the answer to a press would, and what
...
public struct UpdatePanel: Equatable, Sendable {
    ...
    /// published is no news; and after an install that failed it keeps the reason on the row while the
    /// release it finds again makes the button the retry.
