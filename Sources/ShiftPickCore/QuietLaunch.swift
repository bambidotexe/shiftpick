...
public enum QuietLaunch {
    ...
    public static func mark() {
        ...
        try? files.createDirectory(at: Paths.appSupport, withIntermediateDirectories: true)
        files.createFile(atPath: Paths.quietLaunch.path, contents: nil)
    }

    /// True when this launch was an installer's. Reads the marker and removes it, so it counts once: a
    /// second launch, which is the user asking, shows the window as it always did.
    @discardableResult
    public static func consume(now: Date = Date()) -> Bool {
        let url = Paths.quietLaunch
