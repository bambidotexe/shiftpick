...
/// The update window, from the press of Update to the moment the app quits: the release is fetched, then
/// made ready (checked, unpacked, checked again) while the app is still running, so the one thing left for
/// after the quit is to swap two folders. "Install and Relaunch" is enabled in `ready` and nowhere else.
public struct UpdateSession: Equatable, Sendable {
    ...
    /// A response that states no length (`expected` ≤ 0) leaves GitHub's own figure for the asset as the
    /// total.
