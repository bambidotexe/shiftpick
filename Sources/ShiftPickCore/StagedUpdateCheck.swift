...
/// What the copy taken out of a release's disk image has to say about itself before it may replace the
/// running app. Its signature is the platform layer's question; these are the three its `Info.plist`
/// answers.
public enum StagedUpdateCheck {
    ...
    /// nil when the copy may be installed. A running version that does not parse is never replaced: a
    /// binary run outside its bundle has none, and it is not an installed app.
