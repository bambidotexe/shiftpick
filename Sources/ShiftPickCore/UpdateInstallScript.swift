...
/// Everything the helper that finishes an update is told. It runs after the app has quit, so it is told all
/// of it up front, as arguments: nothing is interpolated into the script's text.
public struct UpdateInstallPlan: Equatable, Sendable {
    ...
    /// Seconds: for the app to quit, for the new version to show among the running processes, and how long
    /// after that it is looked for once more. The app itself stops the helper after `K.updateStallNotice`,
    ...
    public init(pid: Int32, destination: URL, staged: URL, backup: URL, resultFile: URL, logFile: URL,
                executableName: String, version: String,
                quitWait: Int = K.updateQuitWait, launchWait: Int = K.updateLaunchWait,
                settle: Int = K.updateSettle) {
        self.pid = pid
    ...
    public var arguments: [String] {
        [String(pid), destination.path, staged.path, backup.path, resultFile.path, logFile.path,
         executableName, version, String(quitWait), String(launchWait), String(settle)]
    }
}

/// What the helper leaves for the next launch to read: one line. The launch that reads it renames the file
/// to `<result>.read`, which is how the helper knows a version that is gone again two seconds later had
/// started properly, and was quit, rather than crashed on its way up.
public enum UpdateResult: Equatable, Sendable {
    ...
    /// `age` is the time since the line was written. Past `K.updateResultShelfLife` it was left behind by
    /// an install nobody is waiting on any more, and opens no window; a negative one means the clock moved,
    /// which proves nothing.
    public static func isNews(age: TimeInterval) -> Bool {
        (0...K.updateResultShelfLife).contains(age)
    }
...
/// The helper itself: a POSIX shell script, run by `/bin/sh` in a process group of its own so that it
/// outlives the app that started it. Everything that can be refused (the download, the disk image, the
/// signature, the version, the folder's permissions) was settled while the app was still running; what is
/// left for after the quit is two renames and a launch, and a way back if the new version does not start.
///
/// The two tools are taken from the environment when it names them, which is how the tests stand in for
/// `open` and `ps`. The app starts the helper with an environment of its own making.
public enum UpdateInstallScript {
    public static let text = #"""
    #!/bin/sh
    #  $1 pid of the app          $5 result file        $9  seconds to wait for the app to quit
    #  $2 the installed bundle    $6 log file           $10 seconds to wait for the new version to show
    #  $3 the new bundle          $7 executable name    $11 seconds after which it is looked for once more
    #  $4 where the previous      $8 the new version
    #     bundle waits
    set -u
    pid="$1"; dest="$2"; staged="$3"; backup="$4"; result="$5"; log="$6"; exe="$7"; version="$8"
    shift 8
    ...
    launch() { "$open_tool" "$dest" >/dev/null 2>&1; }
    ...
    # The app's own quit is what ends it. Until it is gone nothing is touched, and if it never goes nothing
    # is: the app is still there to say so itself, and stops this helper when it does.
    ...
    if launch; then
        ...
        if [ "$seen" -eq 1 ] && [ "$settle" -gt 0 ]; then
            ...
            # business. One that had not either crashed on its way up or was never really there. The second
            # look lasts as long as the first before it is believed.
